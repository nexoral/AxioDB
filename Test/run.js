...
class BasicCRUDTester {
  ...
  log(message, type = 'info') {
    ...
    console.log(colors[type] || '%s', `[${new Date().toISOString()}] ${message}`);
  }
  ...
  async testCollectionCreation() {
    await this.test("Collection creation", async () => {
      ...
      if (!this.basicCollection || typeof this.basicCollection.insert !== 'function') {
  ...
  async testInsertOperations() {
    ...
    await this.test("Multiple documents insert", async () => {
      const result = await this.basicCollection.insertMany(sampleUsers.slice(1));
  ...
  async testFindOperations() {
    ...
    await this.test("Find with $regex operator", async () => {
      await this.basicCollection.query({ 
    ...
    await this.test("Find with setProject", async () => {
      await this.basicCollection.query({}).setProject({ 
        _id: 1, 
        name: 1, 
        email: 1 
      }).exec();
    ...
    await this.test("Chained query methods", async () => {
      await this.basicCollection.query({ age: { $gt: 20 } })
        .Limit(10)
        .Skip(0)
        .Sort({ age: -1 })
        .setCount(true)
  ...
  async testUpdateOperations() {
    await this.test("Update single document", async () => {
      await this.basicCollection.update({ name: "John Doe" })
        .UpdateOne({ age: 31 });
    });

    await this.test("Update multiple documents", async () => {
      await this.basicCollection.update({ age: { $gt: 20 } })
  ...
  async testDeleteOperations() {
    ...
    await this.test("Delete single document", async () => {
      await this.basicCollection.delete({ name: "Delete Test 1" })
        .deleteOne();
    });

    await this.test("Delete multiple documents", async () => {
      await this.basicCollection.delete({ 
  ...
  printSummary() {
    ...
    if (this.testResults.failed === 0) {
      this.log("🎉 ALL TESTS PASSED! AxioDB Basic CRUD is working perfectly!", 'success');
    } else {
