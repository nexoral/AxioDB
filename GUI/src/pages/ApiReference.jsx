    ...
    <div className="container mx-auto px-4 py-8">
      ...
      {apiRoutes.length === 0 && !loading && !error ? (
        <div className="bg-yellow-100 border border-yellow-400 text-yellow-700 px-4 py-3 rounded">
          <p>No API routes found. The server might not have returned any routes.</p>
        </div>
      ) : (
                  ...
                  {group.Paths.map((path, index) => (
                    <div key={index} className="p-4 hover:bg-gray-50">
                      <div className="flex items-start">
                        <span className={`${getMethodColor(path.method)} text-white text-xs font-bold px-2 py-1 rounded mr-3 min-w-16 text-center`}>
                          {path.method}
                        </span>
                        <div className="flex-1">
                          ...
                          {path.payload && (
                            <div>
                              <h4 className="text-sm font-semibold text-gray-700 mt-2 mb-1">Payload:</h4>
