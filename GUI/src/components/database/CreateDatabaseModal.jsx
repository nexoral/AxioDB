...
const CreateDatabaseModal = ({ isOpen, onClose, onDatabaseCreated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${BASE_API_URL}/api/db/create-database`,
        { name: databaseName },
      ...
      setError(error.response?.data?.message || 'Failed to create database. Please try again.');
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 animate-fadeIn">
      <div className="bg-white rounded-lg max-w-md w-full p-6">
        <h3 className="text-xl font-bold text-gray-900 mb-4">Create New Database</h3>
