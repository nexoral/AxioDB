...
const DeleteDatabaseModal = ({ isOpen, dbName, onClose, onConfirmDelete }) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      alert(error.response?.data?.message || 'Failed to delete database. Please try again.');
    } finally {
      setIsDeleting(false);
    }
  }; return (
      ...
      <div className="bg-white rounded-lg max-w-md w-full p-6 animate-fadeIn">
        ...
        <div className="flex justify-end space-x-4">
          ...
          <button
            ...
            disabled={isDeleting}
            className={`px-4 py-2 bg-red-600 text-white rounded hover:bg-red-700 transition-colors flex items-center ${isDeleting ? 'opacity-75 cursor-not-allowed' : ''
              }`}
          >
            {isDeleting ? (
              <>
