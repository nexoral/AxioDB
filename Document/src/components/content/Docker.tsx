...
# Volume: /app is the main data directory`}
          ...
          <div className="mt-4 p-4 bg-sky-900/20 rounded-lg border border-sky-700">
            <p className="text-sm text-sky-200">
              TCP authentication is on by default in this image. Log into the GUI at{" "}
              <code className="px-1 py-0.5 bg-slate-800 rounded">http://localhost:27018</code>{" "}
              as <code className="px-1 py-0.5 bg-slate-800 rounded">admin</code>/
              <code className="px-1 py-0.5 bg-slate-800 rounded">admin</code> to complete the
        ...
        <div className="space-y-6">
          <div className="bg-slate-800 p-6 rounded-xl border border-slate-700">
            ...
            <p className="text-slate-300 mb-4">
              ...
              override any of them with <code className="px-1.5 py-0.5 bg-slate-900 rounded">-e VAR=value</code> at
              <code className="px-1.5 py-0.5 bg-slate-900 rounded ml-1">docker run</code> time, no rebuild required.
            </p>
              ...
              <table className="w-full text-sm text-left">
                <thead>
                  <tr className="border-b border-slate-700">
                    <th className="py-2 pr-4 font-semibold text-white">Variable</th>
                    <th className="py-2 pr-4 font-semibold text-white">Default</th>
            ...
            <p className="text-sm text-slate-400 mt-4">
              ...
              Docker layer with <code className="px-1 py-0.5 bg-slate-900 rounded">-p &lt;host-port&gt;:27018</code> /
              <code className="px-1 py-0.5 bg-slate-900 rounded ml-1">-p &lt;host-port&gt;:27019</code>.
            </p>
          </div>
...
docker run -d --name axiodb-server \\
  ...
  theankansaha/axiodb`}
            />
            <p className="text-sm text-slate-400 mt-4">
              The rule: <code className="px-1 py-0.5 bg-slate-900 rounded">AXIODB_TLS_CERT_PATH</code>{" "}
              ...
              <a href="/cloud#advanced-tls-encryption" className="text-blue-400 hover:underline">AxioDBCloud TLS guide</a>{" "}
          ...
          <div className="bg-slate-800 p-6 rounded-xl border border-slate-700">
            ...
            <p className="text-slate-300 mb-4">
              Mount a volume at <code className="px-1.5 py-0.5 bg-slate-900 rounded">/app</code> so
...
volumes:
  axiodb-data:`}
            ...
            <p className="text-sm text-slate-400 mt-4">
              ...
              <code className="px-1 py-0.5 bg-slate-900 rounded">volumes:</code> below
              - <code className="px-1 py-0.5 bg-slate-900 rounded">./mycerts:/certs:ro</code> is
              your real folder (it contains a <code className="px-1 py-0.5 bg-slate-900 rounded">/</code>),
              mounted read-only; <code className="px-1 py-0.5 bg-slate-900 rounded">axiodb-data:/app</code> is
              a Docker-managed named volume (no <code className="px-1 py-0.5 bg-slate-900 rounded">/</code>,
              just a label):
            </p>
            <CodeBlock
              language="yaml"
