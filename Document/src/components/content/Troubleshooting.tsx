    ...
    body: (
      <>
        <p className="text-slate-700 dark:text-slate-300 mb-4">
          <code className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">client.connect()</code> is
          asynchronous and must be <code className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">await</code>ed
          before you use the connection — it only resolves once the TCP handshake (and, if{" "}
          <code className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">TCPAuth</code> is on, the{" "}
          <code className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">AUTHENTICATE</code> round-trip) has
          completed.
        </p>
        <CodeBlock
          language="javascript"
  ...
  },
  {
    ...
    body: (
      <p className="text-slate-700 dark:text-slate-300">
        You're running with <code className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">TCPAuth: true</code> and
        sent a command before a successful <code className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">AUTHENTICATE</code>.
        Either pass <code className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">{"{ username, password }"}</code> in
        the <code className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">AxioDBCloud</code> constructor (it
        auto-authenticates on <code className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">connect()</code>), or
        call <code className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">await client.login(username, password)</code> yourself
  ...
  },
  {
    ...
    body: (
      <p className="text-slate-700 dark:text-slate-300">
        ...
        change — true for the default <code className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">admin</code>/
        <code className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">admin</code> account, and for any newly
        created user. Log into the GUI at{" "}
        <code className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">http://localhost:27018</code>, sign in,
  ...
  },
  {
    ...
    body: (
      <p className="text-slate-700 dark:text-slate-300">
        ...
        <code className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">config</code> — that name is reserved
        for AxioDB's own RBAC storage (<code className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">users</code>/
        <code className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">roles</code>/
        <code className="px-1.5 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">permissions</code>) and is blocked on
  ...
  },
  {
    ...
    body: (
      <ul className="text-sm text-slate-700 dark:text-slate-300 space-y-2">
        <li>
          • Confirm the server was started with <code className="px-1 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">TCP: true</code> (or,
          in Docker, <code className="px-1 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">AXIODB_TCP=true</code>, the default).
        </li>
        <li>
          • Confirm the port is published: <code className="px-1 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">-p 27019:27019</code> on{" "}
          <code className="px-1 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">docker run</code>, or that nothing else on
  ...
  },
  {
    ...
    body: (
      <p className="text-slate-700 dark:text-slate-300">
        ...
        <code className="px-1 py-0.5 bg-slate-100 dark:bg-slate-900 rounded">docker logs</code>, port-conflict
    ...
    <div className="space-y-12">
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-amber-50 via-white to-orange-50 dark:from-amber-900/20 dark:via-slate-800 dark:to-orange-900/20 rounded-2xl p-8 lg:p-12 border border-amber-200 dark:border-amber-700 shadow-xl">
        ...
        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-gradient-to-r from-amber-100 to-orange-100 dark:from-amber-900/40 dark:to-orange-900/40 rounded-full border border-amber-300 dark:border-amber-600 mb-6">
          ...
          <h1 className="text-5xl lg:text-6xl font-extrabold mb-6 bg-gradient-to-r from-amber-600 via-orange-600 to-red-600 dark:from-amber-400 dark:via-orange-400 dark:to-red-400 bg-clip-text text-transparent">
            Troubleshooting
          </h1>
