    ...
    body: (
      <>
        <p className="text-gray-600 mb-4">
          <code className="px-1.5 py-0.5 bg-white rounded">client.connect()</code> is
          asynchronous and must be <code className="px-1.5 py-0.5 bg-white rounded">await</code>ed
          before you use the connection — it only resolves once the TCP handshake (and, if{" "}
          <code className="px-1.5 py-0.5 bg-white rounded">TCPAuth</code> is on, the{" "}
          <code className="px-1.5 py-0.5 bg-white rounded">AUTHENTICATE</code> round-trip) has
          completed.
        </p>
        <CodeBlock
          language="javascript"
  ...
  },
  {
    ...
    body: (
      <p className="text-gray-600">
        You're running with <code className="px-1.5 py-0.5 bg-white rounded">TCPAuth: true</code> and
        sent a command before a successful <code className="px-1.5 py-0.5 bg-white rounded">AUTHENTICATE</code>.
        Either pass <code className="px-1.5 py-0.5 bg-white rounded">{"{ username, password }"}</code> in
        the <code className="px-1.5 py-0.5 bg-white rounded">AxioDBCloud</code> constructor (it
        auto-authenticates on <code className="px-1.5 py-0.5 bg-white rounded">connect()</code>), or
        call <code className="px-1.5 py-0.5 bg-white rounded">await client.login(username, password)</code> yourself
  ...
  },
  {
    ...
    body: (
      <p className="text-gray-600">
        ...
        change — true for the default <code className="px-1.5 py-0.5 bg-white rounded">admin</code>/
        <code className="px-1.5 py-0.5 bg-white rounded">admin</code> account, and for any newly
  ...
  },
  {
    ...
    body: (
      <p className="text-gray-600">
        ...
        <code className="px-1.5 py-0.5 bg-white rounded">config</code> — that name is reserved
        for AxioDB's own RBAC storage (<code className="px-1.5 py-0.5 bg-white rounded">users</code>/
        <code className="px-1.5 py-0.5 bg-white rounded">roles</code>/
        <code className="px-1.5 py-0.5 bg-white rounded">permissions</code>) and is blocked on
  ...
  },
  {
    ...
    body: (
      <ul className="text-sm text-gray-600 space-y-2">
        <li>
          • Confirm the server was started with <code className="px-1 py-0.5 bg-white rounded">TCP: true</code> (or,
          in Docker, <code className="px-1 py-0.5 bg-white rounded">AXIODB_TCP=true</code>, the default).
        </li>
        <li>
          • Confirm the port is published: <code className="px-1 py-0.5 bg-white rounded">-p 27019:27019</code> on{" "}
          <code className="px-1 py-0.5 bg-white rounded">docker run</code>, or that nothing else on
    ...
    body: (
      <p className="text-gray-600">
        ...
        <a
          ...
          className="inline-flex items-center gap-1 text-sky-600 underline font-medium"
        >
          Docker/README.md <ExternalLink className="h-3.5 w-3.5" />
        </a>{" "}
        in the repository for a fuller Docker-specific troubleshooting guide (
        <code className="px-1 py-0.5 bg-white rounded">docker logs</code>, port-conflict
    ...
    <div className="space-y-12">
      ...
      <section className="relative overflow-hidden bg-gray-100 rounded-lg p-5 sm:p-8 lg:p-12 border border-amber-200 shadow-md">
        ...
        <div className="relative z-10">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-orange-50 rounded-full border border-amber-200 mb-6">
          ...
          <h1 className="text-3xl sm:text-4xl md:text-5xl lg:text-6xl font-extrabold mb-6 text-orange-600">
            Troubleshooting
          </h1>
