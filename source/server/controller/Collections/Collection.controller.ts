...
export default class CollectionController {
    ...
    public async createCollection(
      ...
      try {
        await databaseInstance.createCollection(
          collectionName, crypto, key);
        return buildResponse(StatusCodes.CREATED, "Collection created successfully");
      } catch (error) {
        console.error("Error creating collection:", error);
        return buildResponse(StatusCodes.INTERNAL_SERVER_ERROR, "Failed to create collection");
      }
    }
}